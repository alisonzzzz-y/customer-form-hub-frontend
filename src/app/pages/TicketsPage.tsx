...
export function TicketsPage({
  ...
  const visible = tickets
    .filter((t) => {
      ...
      if (filters.urgency !== "All" && t.urgency !== filters.urgency)
        return false;
  ...
  const smePending = (id: string) =>
    smeRequests.filter(
      (r) => r.ticketId === id && !["Returned", "Closed"].includes(r.status),
    ).length;
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                actions.logActivity(
                  "Imported ticket from mock Salesforce SOR",
                  id,
                );
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-8 py-7 flex flex-col gap-5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <FilterSelect
            label="NDA"
            value={filters.nda}
            options={["In Place", "Missing", "Unknown"]}
